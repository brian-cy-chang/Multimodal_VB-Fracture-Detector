...
class JointFusion_CNN(nn.Module):
    ...
    def __init__(self, batch_size, bert_dim, vb_dim, pt_dem_dim, hidden_size, dropout_rate=0.5):
        ...
        self.pt_dem = nn.Linear(self.pt_dem_dim,8)
        
        bert_size = 4*self.channel_4
        vb_size = self.batch_size
        pt_dem_size = self.batch_size
        
        self.classifier = nn.Linear(bert_size+8+8, self.batch_size)
...
class JointFusion_Transformer(nn.Module):
    def __init__(self, batch_size, bert_dim, in_channels, vb_dim, pt_dem_dim, hidden_size, bert_mode, dropout_rate=0.5):
...
class JointFusion_CNN_Losses_NoReshape_Attention(nn.Module):
    """
    Joint fusion model that returns independent outputs for each modality
        to be used for backpropagating individual losses 
    """
    def __init__(self, batch_size, bert_dim, vb_dim, pt_dem_dim, hidden_size, dropout_rate=0.5):
        super(JointFusion_CNN_Losses_NoReshape_Attention, self).__init__()
        self.batch_size = batch_size
        self.bert_dim = bert_dim
        self.vb_dim = vb_dim
        self.pt_dem_dim = pt_dem_dim
        ...
        self.attention_bert = Attention(self.batch_size, self.channel_2)
        self.attention_vb = Attention(self.batch_size, self.channel_2)
        self.attention_pt_dem = Attention(self.batch_size, self.channel_2)
    ...
    def forward(self, x1, x2, x3):
        ...
        combined_attn_weights = self.attention_bert(combined_features)
        combined_features = combined_features * combined_attn_weights
